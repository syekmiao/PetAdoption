        ...
        if (count($errors) > 0){
            ...
            elseif($filesize > 1000000000){
                ...
                $sql = "INSERT INTO pets
                        (petName, petType, petPicture, petAge, petBreed, vaccination, deworm, spray, petSize, petColor, petInformation,petStatus, userID)
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                    $stmt = mysqli_stmt_init($conn);
                    $preparestmt = mysqli_stmt_prepare($stmt, $sql);
                    if ($preparestmt){
                        mysqli_stmt_bind_param($stmt, "sssssssssssss", 
                            $petname, $petType, $newImageName, $petage, $breed, $vaccinate, $deworm, $spray, $size, $color, $information, $status, $userId);
